...
public class LibraryViewController extends Controller {
	...
	private LibraryViewController(Controller fallback) {
		...
		add("savetoserver", new CommandFactoryCheckLib() {
			@Override
			public Command create() {
				return new Command(false) {
					@Override
					public void execute() {
						try {
							...
							File workingDirectory = new File(System.getProperty("user.dir"));
							fc.setCurrentDirectory(workingDirectory);
							fc.setFileFilter(new ServerFileFilter());
							fc.setAcceptAllFileFilterUsed(false);
							int rv = fc.showSaveDialog(null);
							if(rv == JFileChooser.APPROVE_OPTION){
								File file = fc.getSelectedFile();
								String f = ServerFileFilter.appendExtensionIfRequired(file).getPath();
								library.saveToServerFile(f);
							}
							view.setTitleText(library.getFileName());
						} catch (FileNotFoundException e) {
							showError("cannot save : file not found");
