...
public class ComponentType {
	...
	protected static final Color LIGHT_GREY = Color.decode("#c0c0c0");
	...
	protected List<ConnectionInfo> inputList = new LinkedList<ConnectionInfo>();
	/** A list of the outputs we have - gets changed to an array later */
	protected List<ConnectionInfo> outputList = new LinkedList<ConnectionInfo>();
	 ...
	 *            position in canvas
	 */
	Component create(Patch p,Point pos, int page) {
		return new Component(p,this, pos, page);
	}
	 ...
	 * @param selected
	 * @param c
	 * @param curpage the current visible page
	 * @throws ConnectionOutOfRangeException 
	 */
	void drawConnections(Graphics g, boolean selected, Component c, int curpage) throws ConnectionOutOfRangeException {
		for (int i = 0; i < getInputCount(); i++) {
			Component.Input inp = c.getInput(i);
			if (inp != null) {
				if(inp.c.page == curpage || c.page == curpage){ // one or another end is visible
					Point p1,p2,boxp=null;
					g.setColor(getInputType(i).getCol());
					if(inp.c.page == curpage && c.page != curpage){
						// destination is off-page
						p2 = inp.c.getType().getOutputPos(inp.output, inp.c);
						p1 = new Point(p2.x,p2.y+20);
						boxp=p1;
					} else if(inp.c.page != curpage && c.page == curpage){
						// source is off-page
						p1 = getInputPos(i, c);
						p2 = new Point(p1.x,p1.y-20);
						boxp=p2;
					} else {
						// both are on-page
						p1 = getInputPos(i, c);						
					...
					if(boxp!=null){
						g.fillRect(boxp.x-5, boxp.y-5, 11, 11);
					}
				}
			}
		}
